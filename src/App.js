...
function isUser() {
  return getRoleFromToken() === "User Mới";
}
        ...
        {buttons.map(({ path, label, isLogout, onClick }) => {
          ...
          return (
            <button
              ...
              className={
                isLogout
                  ? 'buttonPath-head-logout'
                  : isActive
                    ? 'buttonPath-head active'
                    : 'buttonPath-head'
              }
      ...
      <ProtectedRedirect>
        <Routes>
          ...
          <Route path="/loginUser" element={
            isAuthenticated() ? <Navigate to="/" replace /> : <LoginUser />
          } />

          <Route path="/signInPhone" element={
            isAuthenticated() ? <Navigate to="/" replace /> : <SignInPhone />
          } />
          <Route path="/phoneVerification" element={
            isAuthenticated() ? <Navigate to="/" replace /> : <PhoneVerification />
          } />
          <Route path="/signInEmail" element={
            isAuthenticated() ? <Navigate to="/" replace /> : <SignInEmail />
          } />
          <Route path="/emailVerification" element={
            isAuthenticated() ? <Navigate to="/" replace /> : <EmailVerification />
          } />
          ...
          <Route path="/profile" element={
            isAuthenticated() ? <Profile /> : <Navigate to="/loginUser" replace />
          } />

          <Route path="/setUserAccount" element={
            isAuthenticated()
              ? isUser()
